...
class GBNHost():
    ...
    def receive_from_network_layer(self, packet):
        ...
        if not self.is_corrupt(packet):
            ...
            if ackNum != self.expected_seq_num:
                self.simulator.pass_to_network_layer(self.entity, self.last_ack_pkt)
        
        elif self.is_corrupt(packet):
            self.simulator.pass_to_network_layer(self.entity, self.last_ack_pkt)
        elif ackNum != self.expected_seq_num:
            self.simulator.pass_to_network_layer(self.entity, self.last_ack_pkt)
        else:
            self.simulator.pass_to_application_layer(self.entity, data)
            self.last_ack_pkt = self.create_ack_pkt(self.expected_seq_num, ackNum)
            self.simulator.pass_to_network_layer(self.entity, self.last_ack_pkt)
    ...
    def timer_interrupt(self):
        ...
        for a in range(self.window_base, self.next_seq_num, 1):
            self.simulator.pass_to_network_layer(self.entity, self.unacked_buffer[a])
    ...
    def create_data_pkt(self, seq_num, payload):
        ...
        cur_pack = pack("!HIHI"+length+"s", 0x0, seq_num, 0, len(payload), payload.encode())

        check_sum = self.create_checksum(cur_pack)
        cur_pack = pack("!HIHI"+length+"s", 0x0, seq_num, check_sum, len(payload), payload.encode())
    ...
    def create_ack_pkt(self, seq_num):
        ...
        Returns:
            bytes: a bytes object containing the required fields for a data packet
        """
        cur_pack = pack("!HIH", 0x1, seq_num, 0)
        check_sum = self.create_checksum(cur_pack)
        cur_pack = pack("!HIH", 0x1, seq_num, check_sum)
    ...
    def unpack_pkt(self, packet):
        ...
        """
        type = unpack("!H", packet[:2])

        if type[0] == 0x0:
            header = unpack("!HiHI", packet[:12])
            pl = unpack("!%is" % header[3], packet[12:])[0]
    ...
    def is_corrupt(self, packet):
        ...
        """
        packet_length = unpack("!HIHI", packet[:12])
        check_sum = packet_length[2]
        header = pack("!HIHI", packet_length[0], packet_length[1], 0, packet_length[3])
        cur_pack = header + packet[12:]
        if check_sum == self.create_checksum(cur_pack):
            return False
